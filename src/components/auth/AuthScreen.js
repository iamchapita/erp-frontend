...
const LoginScreen = () => {
	...
	const isFormValid = () => {
		if (validator.isEmpty(email)) {
			...
			dispatch(
				uiSetError("La contraseña debe contener al menos 8 caracteres.")
			);
			return false;
		}
	...
	return (
		...
		>
			<div className="relative rounded space-y-4 opacity-90 flex flex-col bg-custom-400  font-normal p-4 w-80 ">
				...
				<SocialNetworkBottom
					...
					handleAction={handleGoogleLogin}
				/>
				<Link
					to={"/register"}
					className={
						"underlinetransition-all cursor-pointer self-start hover:text-custom-500"
					}
				>
					¿No tienes una cuente? Regístrate
				</Link>
			</div>
		</form>
	);
